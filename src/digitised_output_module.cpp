...
using ts_histogram = bh::histogram<std::tuple<bh::axis::regular<>>,
                                   bh::dense_storage<bh::accumulators::count<double, true>>>;

ts_histogram make_ts_histogram(int nbins, double low, double high) {
    return bh::make_histogram_with(bh::dense_storage<bh::accumulators::count<double, true>>{},
                                   bh::axis::regular<>(nbins, low, high));
}

std::unique_ptr<TH1D> to_root(ts_histogram const& h, char const* name, char const* title) {
    auto const& ax = h.axis();
    auto root_h = std::make_unique<TH1D>(name, title, static_cast<int>(ax.size()), ax.value(0),
                                         ax.value(ax.size()));
    for (int i = 0; i < static_cast<int>(ax.size()); ++i)
        root_h->SetBinContent(i + 1, h.at(i).value());
    root_h->SetBinContent(0, h.at(-1).value());
    root_h->SetBinContent(static_cast<int>(ax.size()) + 1, h.at(ax.size()).value());
    root_h->ResetStats();
    return root_h;
}
...
class MCRNTupleWriter {
   public:
    explicit MCRNTupleWriter(std::string filename) {
        ...
        writer_ = RNTupleParallelWriter::Recreate(std::move(model), "events", filename);
    }
    ...
    void write(std::vector<SHiP::MCParticle> const& particles) {
        ...
        *state.entry->GetPtr<std::vector<SHiP::MCParticle>>("mc_particles") = particles;
...
class SimRNTupleWriter {
   public:
    ...
    void write(std::vector<SHiP::MCParticle> const& particles, SHiP::SimResult const& result) {
        ...
        *state.entry->GetPtr<std::vector<SHiP::SimParticle>>("sim_particles") = result.particles;
        state.ctx->Fill(*state.entry);
    }
   ...
   public:
    ...
    void observe(std::vector<SHiP::MCParticle> const& particles) {
        ...
        for (auto const& p : particles) {
            ...
            double pmag = std::sqrt(mom[0] * mom[0] + mom[1] * mom[1] + mom[2] * mom[2]);
            h_momentum_(pmag);
            h_pdg_(p.pdgCode);
        }
    }
    ...
    ~MCHistogrammer() {
        ...
        file->cd();
        file->WriteTObject(
            to_root(h_multiplicity_, "h_mc_multiplicity", "MC particles per event;N;Events").get());
        file->WriteTObject(
            to_root(h_momentum_, "h_mc_momentum", "MC particle |p|;|p| [GeV/c];Entries").get());
        file->WriteTObject(to_root(h_pdg_, "h_mc_pdg", "MC particle PDG code;PDG;Entries").get());
        file->Close();
    }
...
class SimHistogrammer {
   public:
    ...
    void observe(std::vector<SHiP::MCParticle> const& particles, SHiP::SimResult const& result) {
        ...
        for (auto const& hit : result.hits) {
            ...
            h_hit_momentum_(std::sqrt(mom[0] * mom[0] + mom[1] * mom[1] + mom[2] * mom[2]));
        }
    ...
    ~SimHistogrammer() {
        ...
        file->cd();
        file->WriteTObject(
            to_root(h_mc_multiplicity_, "h_mc_multiplicity", "MC particles per event;N;Events")
                .get());
        file->WriteTObject(
            to_root(h_mc_momentum_, "h_mc_momentum", "MC particle |p|;|p| [GeV/c];Entries").get());
        file->WriteTObject(
            to_root(h_hit_multiplicity_, "h_hit_multiplicity", "Sim hits per event;N;Events")
                .get());
        file->WriteTObject(
            to_root(h_hit_edep_, "h_hit_edep", "Hit energy deposit;E_{dep} [GeV];Entries").get());
        file->WriteTObject(to_root(h_hit_z_, "h_hit_z", "Hit z position;z [mm];Entries").get());
        file->WriteTObject(
            to_root(h_hit_detector_, "h_hit_detector", "Hit detector ID;Detector ID;Entries")
                .get());
        file->WriteTObject(
            to_root(h_hit_momentum_, "h_hit_momentum", "Hit |p|;|p| [GeV/c];Entries").get());
        file->WriteTObject(to_root(h_particle_multiplicity_, "h_particle_multiplicity",
   ...
   private:
    ...
    ts_histogram h_hit_multiplicity_, h_hit_edep_, h_hit_z_, h_hit_detector_, h_hit_momentum_;
...
PHLEX_REGISTER_ALGORITHMS(m, config) {
    ...
    auto rntuple_file = config.get<std::string>("rntuple_file", std::string{"sim_output.root"});
    auto histo_file = config.get<std::string>("histo_file", std::string{"validation.root"});

    if (mode != "mc_only" && mode != "full" && mode != "noop" && mode != "noop_full")
        throw std::runtime_error("Unknown sim_output_module mode: '" + mode +
                                 "' (expected 'mc_only', 'full', 'noop', or 'noop_full')");

    if (mode == "noop") {
        auto noop = m.make<MCNoop>();
        noop.observe("noop", &MCNoop::observe, concurrency::unlimited)
            .input_family(product_selector{.creator = "mc_particles"_id, .layer = "event"_id});
        ...
        noop.observe("noop", &SimNoop::observe, concurrency::unlimited)
            .input_family(
                ...
                product_selector{
                    .creator = "geant4"_id, .layer = "event"_id, .suffix = "sim_result"_id});
    } else if (mode == "mc_only") {
        auto writer = m.make<MCRNTupleWriter>(rntuple_file);
        writer.observe("write_rntuple", &MCRNTupleWriter::write, concurrency::unlimited)
            .input_family(product_selector{.creator = "mc_particles"_id, .layer = "event"_id});

        auto histogrammer = m.make<MCHistogrammer>(histo_file);
        histogrammer.observe("validate", &MCHistogrammer::observe, concurrency::unlimited)
            .input_family(product_selector{.creator = "mc_particles"_id, .layer = "event"_id});
    } else {
        auto filter_empty = config.get<bool>("filter_empty", false);

        auto writer = m.make<SimRNTupleWriter>(rntuple_file, filter_empty);
        writer.observe("write_rntuple", &SimRNTupleWriter::write, concurrency::unlimited)
            .input_family(
                product_selector{.creator = "mc_particles"_id, .layer = "event"_id},
                product_selector{
                    .creator = "geant4"_id, .layer = "event"_id, .suffix = "sim_result"_id});
        ...
        histogrammer.observe("validate", &SimHistogrammer::observe, concurrency::unlimited)
            .input_family(
                product_selector{.creator = "mc_particles"_id, .layer = "event"_id},
                product_selector{
                    .creator = "geant4"_id, .layer = "event"_id, .suffix = "sim_result"_id});
