...
class StrawTubes {
   public:
    explicit StrawTubes(std::mt19937& rng, double sigma_x = 0.1, double sigma_y = 0.1)
        : rng_(rng), dx_(0.0, sigma_x), dy_(0.0, sigma_y) {}

    ::SHiP::StrawTubesHit digitise(::SHiP::SimHit const& sim_hit) {
        auto reconstructed = SHiP::fromSimHit(sim_hit);
        reconstructed.position = {sim_hit.position[0] + dx_(rng_), sim_hit.position[1] + dy_(rng_),
                                  sim_hit.position[2]};

        return ::SHiP::StrawTubesHit{.recHit = reconstructed};
    }
